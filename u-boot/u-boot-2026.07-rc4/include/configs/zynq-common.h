...
#define CFG_EXTRA_ENV_SETTINGS	\
    ...
    "uenvboot=if run sd_uEnvtxt_existence_test; then " \
        "run loadbootenv; " \
    ...
    "if test -n $uenvcmd; then " \
        ...
        "run uenvcmd; " \
    "fi\0" \
    "cp_kernel2ram=mmcinfo && fatload mmc ${sdbootdev} 0x10000000 image.ub\0" \
    "default_bootcmd=run uenvboot; run cp_kernel2ram && bootm 0x10000000\0" \
    BOOTENV
#endif

#undef CFG_BOOTCOMMAND
#define CFG_BOOTCOMMAND "run default_bootcmd"
